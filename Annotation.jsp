     ...
     -->
			...
			<form action='<c:url value="/Annotation/RequestParamMap.do"/>'>
				...
				성별: <input type="radio" name="gender" value="남자" />남자 
					 <input type="radio" name="gender" value="여자" />여자 <input type="submit" value="확인" />
			</form>
		</fieldset>
		<hr />
		<fieldset>
			...
			<form action="<c:url value='/Annotation/ModelAttribute.do'/>" method="post">
				<table bgcolor="gray" cellspacing="1" width="60%">
					...
					<tr bgcolor="white">
						<td>관심사항</td>
						<td><input type="checkbox" name="inters" value="정치" />정치 
						<input type="checkbox" name="inters" value="경제" />경제 
						<input type="checkbox" name="inters" value="스포츠" />스포츠</td>
					</tr>
					<tr bgcolor="white">
						<td>학력</td>
		...
		<fieldset>
			...
			<br /> <a href="<c:url value="/Annotation/isLogin.do"/>">로그인 여부 판단</a><br/>
			\${param.error } : ${param.error }<br/>
			\${param.error2 } : ${param.error2 }<br/>
			<ul>
				<li>서블릿 API(HttpSession사용시) :${isLoginMessage }</li>
				<li>@SessionAttribute사용시 :${not empty param.error ? "비 로그인 상태":""}${isLoginMessage }
